...
import { ReactNode, useCallback, useState, FormEvent, } from "react";
...
const SUGGESTIONS = [
  ...
  "Create a meeting on 20th aug and keep the time as 10am for 30 minutes and keep saifulpurkait123@gmail.com as attendee",
];
...
function ChatPanel({sessionToken, connections, footer}: Props) {
  ...
  const bottomRef = useRef<HTMLDivElement>(null); 
   ...
   useEffect(() => {                                          // 👈 ADD IT HERE
    bottomRef.current?.scrollIntoView({ behavior: "smooth", block: "end" });
  }, [messages, progress, running]);
      ...
      <section className={styles.main}>
        ...
        <div className={styles.chatColumn}>
          ...
          <div className={styles.composerWrap}>
            <form onSubmit={onSubmit} action="" className={styles.composerForm}>
              <Textarea 
              value={prompt}
              ...
              />
              <Button type="submit" size="icon" disabled={!prompt.trim() || running} 
              className={styles.sendBtn}
              area-label="Send text messages"
