...
import { getThreadMessages, listUserThreads, streamAgentReply } from "../services/agent.service";
...
agentRoutes.get('/threads', async(req, res)=>{
    ...
    try{
        ...
        const message = error instanceof Error ? error. message: "Failed to new threads"
...
agentRoutes.get('/threads/:threadId', async(req, res)=>{
    ...
    const parsed = threadsIdSchema.safeParse(req.params.threadId);
    if(!parsed.success){
